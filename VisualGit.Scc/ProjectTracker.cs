// VisualGit.Scc\ProjectTracker.cs
//
...
{
    interface IVisualGitProjectDocumentTracker
    {
        void Hook(bool enable);
    }

    //[CLSCompliant(false)]
    [GlobalService(typeof(IVisualGitProjectDocumentTracker))]
    partial class ProjectTracker : VisualGitService, IVisualGitProjectDocumentTracker, IVsTrackProjectDocumentsEvents2, IVsTrackProjectDocumentsEvents3
    {
        bool _hooked;
        uint _projectCookie;
        uint _documentCookie;
        VisualGitSccProvider _sccProvider;
        VisualGitSccSettingStorage _sccStore;
        bool _collectHints;
        ...
        public ProjectTracker(IVisualGitServiceProvider context)
            : base(context)
        {
        ...
        {
            [DebuggerStepThrough]
            get { return _sccProvider ?? (_sccProvider = GetService<VisualGitSccProvider>()); }
        }

        VisualGitSccSettingStorage SccStore
        {
            [DebuggerStepThrough]
            get { return _sccStore ?? (_sccStore = GetService<VisualGitSccSettingStorage>(typeof(ISccSettingsStore))); }
        }
        ...
        void RegisterForSccCleanup()
        {
            ...
            Context.GetService<IVisualGitCommandService>().PostTickCommand(ref _registeredSccCleanup, VisualGitCommand.SccFinishTasks);
        }
    }
}
