// VisualGit.Scc\VisualGitSccProvider.Glyphs.cs
//
...
namespace VisualGit.Scc
{
    ...
    partial class VisualGitSccProvider : IVsSccManager2, IVsSccManagerTooltip, IVsSccGlyphs
    {
        ...
        public VisualGitGlyph GetPathGlyph(string path)
        {
            ...
            if (rf != null)
            {
                foreach (string fn in rf.GetSubFiles())
                {
                    if (IsChildChanged(fn))
                        return VisualGitGlyph.ChildChanged;
                }
        ...
        uint GlyphToStatus(VisualGitGlyph glyph)
        {
            SccStatus status;
            switch (glyph)
            {
        ...
        public int GetSccGlyph(int cFiles, string[] rgpszFullPaths, VsStateIcon[] rgsiGlyphs, uint[] rgdwSccStatus)
        {
            ...
            for (int i = 0; i < cFiles; i++)
            {
                ...
                VisualGitGlyph glyph = GetPathGlyph(file);
        ...
        public void UpdateSolutionGlyph()
        {
            ...
            VisualGitGlyph glp = GetPathGlyph(sf);
        ...
        void ClearSolutionGlyph()
        {
            ...
            hier.SetProperty(VSConstants.VSITEMID_ROOT, (int)__VSHPROPID.VSHPROPID_StateIconIndex, (int)VisualGitGlyph.Blank);
        }
