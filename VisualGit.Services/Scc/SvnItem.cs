...
namespace VisualGit
{
    ...
    public sealed partial class SvnItem : ISvnItemUpdate, ISvnWcReference, IEquatable<SvnItem>
    {
        ...
        public SvnItem(IFileStatusCache context, string fullPath, VisualGitStatus status)
        {
            if (context == null)
        ...
        void ScheduleUpdateNotify()
        {
            ...
            if (cs != null)
                cs.PostTickCommand(ref _scheduled, VisualGitCommand.TickRefreshSvnItems);
        }

        void ISvnItemUpdate.RefreshTo(VisualGitStatus status)
        {
            _ticked = false;
            RefreshTo(status);
        }
