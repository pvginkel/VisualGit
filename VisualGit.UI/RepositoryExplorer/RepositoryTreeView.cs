...
namespace VisualGit.UI.RepositoryExplorer
{
    ...
    partial class RepositoryTreeView : TreeViewWithSelection<RepositoryTreeNode>
    {
        ...
        internal void BrowseItem(Uri uri)
        {
            ...
            VisualGitAction d = delegate()
            {
                ...
                try
                {
                    ...
                    VisualGitAction addItems = (VisualGitAction)delegate()
                        {
                ...
                {
                    if (!ok)
                        BeginInvoke((VisualGitAction)delegate()
                        {
                            _running.Remove(nUri);
        ...
        internal void OnAfterEdit(RepositoryExplorerItem item, string newName, CancelEventArgs e)
        {
            ...
            CommandResult cr = Context.GetService<IVisualGitCommandService>().DirectlyExecCommand(VisualGitCommand.RenameRepositoryItem, new string[] { newName });
        }
        ...
        internal void DoCreateDirectory()
        {
            ...
            using (VisualGit.UI.SccManagement.CreateDirectoryDialog dialog = new VisualGit.UI.SccManagement.CreateDirectoryDialog())
            {
