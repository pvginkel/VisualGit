// VisualGit.VS\SolutionExplorer\SolutionTreeViewManager.cs
//
...
{
    sealed class SolutionTreeViewManager : VisualGitService
    {
        bool _useVisualGitIcons;
        IntPtr _originalImageList;

        Win32TreeView _treeViewControl;

        public SolutionTreeViewManager(IVisualGitServiceProvider context)
            : base(context)
        {
        }
        ...
        public void SetVisualGitIcons(bool enabled)
        {
            if (enabled == _useVisualGitIcons)
            {
                if (enabled)
                    SetIcons();
