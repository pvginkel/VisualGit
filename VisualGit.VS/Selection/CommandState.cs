// VisualGit.VS\Selection\CommandState.cs
//
...
{
    [GlobalService(typeof(IVisualGitCommandStates))]
    sealed class CommandState : VisualGitService, IVisualGitCommandStates, IVsShellPropertyEvents
    {
        ...
        public CommandState(IVisualGitServiceProvider context)
            : base(context)
        {
        }
        ...
        CmdStateCacheItem _visualGitActiveScc;
        public bool SccProviderActive
        {
            get { return (_visualGitActiveScc ?? (_visualGitActiveScc = GetCache(new Guid(VisualGitId.SccProviderId)))).Active; }
        }
        ...
        #region IVisualGitCommandStates Members
            ...
            try
            {
                foreach (SccData scc in _otherSccProviderContexts)
                {
                    if (scc.Active)
                    {
                        ...
                        IVsSccProvider pv = GetService<IVisualGitQueryService>().QueryService<IVsSccProvider>(gService);
            ...
            if (_otherSccProviderContexts == null)
            {
                ...
                if (ErrorHandler.Succeeded(lr.GetLocalRegistryRoot(out root)))
                {
                    ...
                    using (RegistryKey rk = baseKey.OpenSubKey(root + "\\SourceControlProviders", RegistryKeyPermissionCheck.ReadSubTree))
                    {
                        if (rk != null)
                        {
                            string myId = VisualGitId.SccProviderGuid.ToString("B");
            ...
            switch ((__VSSPROPID)propid)
            {
                case __VSSPROPID.VSSPROPID_Zombie:
                    if (var is bool)
                    {
                        ...
                        if (!_zombie)
                            GetService<IVisualGitServiceEvents>().OnUIShellActivate(EventArgs.Empty);
                    }
                    break;
            }
