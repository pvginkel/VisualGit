// VisualGit.VS\LanguageServices\Core\VisualGitLanguageDropDownBar.cs
//
...
{
    public class VisualGitLanguageDropDownBar : VisualGitService, IVsDropdownBarClient, IVisualGitIdleProcessor
    {
        VisualGitCodeWindowManager _manager;
        bool _added;
        ...
        public VisualGitLanguageDropDownBar(VisualGitLanguage language, VisualGitCodeWindowManager manager)
            : base(language)
        {
            if (manager == null)
        ...
        protected VisualGitCodeWindowManager Manager
        {
            get { return _manager; }
        }
        ...
        internal void Close()
        {
            ...
            try
            {
                if (_added)
                {
                    GetService<IVisualGitPackage>().UnregisterIdleProcessor(this);
                    IVsDropdownBarManager dbm = Manager.CodeWindow as IVsDropdownBarManager;
        ...
        void IVisualGitIdleProcessor.OnIdle(VisualGitIdleArgs e)
        {
            OnIdle(e);
        ...
        protected virtual void OnIdle(VisualGitIdleArgs e)
        {
        }
        ...
        internal void OnDataUpdated(ComboMemberCollection comboMemberCollection,VisualGitLanguageDropDownBar _bar,int _index)
        {
            _shouldUpdate = true;
        }

        class ComboTextView : VisualGitService, IVsTextViewEvents, IDisposable
        {
            IVsTextView _view;
            uint _cookie;

            public ComboTextView(VisualGitLanguageDropDownBar bar, IVsTextView view)
                : base(bar)
            {
                _view = view;
            ...
            protected VisualGitLanguageDropDownBar Bar
            {
                get { return (VisualGitLanguageDropDownBar)Context; }
            }
        ...
        public class ComboMemberCollection : Collection<ComboMember>
        {
            ...
            public ComboMemberCollection(VisualGitLanguageDropDownBar bar, int index)
            {
                _bar = bar;
                _index = index;
                _current = -1;
