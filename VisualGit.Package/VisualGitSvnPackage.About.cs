...
namespace VisualGit.VSPackage
{
    ...
    public partial class VisualGitSvnPackage : IVsInstalledProduct
    {
        ...
        private Version GetUIVersion()
        {
            ...
            using (RegistryKey rk = baseKey.OpenSubKey(root + "\\Packages\\" + typeof(VisualGitSvnPackage).GUID.ToString("b"), RegistryKeyPermissionCheck.ReadSubTree))
            {
                if(rk == null)
                    return null;
        ...
        {
            get { return _packageVersion ?? (_packageVersion = typeof(VisualGitSvnPackage).Assembly.GetName().Version); }
        }
