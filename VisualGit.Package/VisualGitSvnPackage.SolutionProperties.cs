...
{
    [ProvideSolutionProperties(VisualGitSvnPackage.SubversionPropertyCategory)]
    [ProvideSolutionProperties(VisualGitId.SccStructureName)]
    //[ProvideSolutionProperties(VisualGitId.IssueTrackerStructureName)]
    partial class VisualGitSvnPackage : IVsPersistSolutionProps
    {
        const string SubversionPropertyCategory = VisualGitId.SubversionSccName;
        ...
        public bool LoadUserProperties(string streamName)
        {
            ...
            return ErrorHandler.Succeeded(ps.LoadPackageUserOpts((IVsPersistSolutionOpts)GetService<VisualGit.UI.IVisualGitPackage>(), streamName));
        }
        ...
        public int SaveSolutionProps(IVsHierarchy pHierarchy, IVsSolutionPersistence pPersistence)
        {
            ...
            if (pHierarchy == null) // Only save the property on the solution itself
            {
                ...
                if (translate != null && translate.HasSolutionData)
                    pPersistence.SavePackageSolutionProps(1 /* TRUE */, null, this, VisualGitId.SccStructureName);
            ...
            using (IPropertyMap map = translate.GetMap(pPropBag))
            {
                switch (pszKey)
                {
                    case SubversionPropertyCategory:
                        ...
                        map.SetRawValue(ManagerPropertyName, "VisualGit - Subversion Support for Visual Studio");
        ...
        const string SccPendingChangeStream = VisualGitId.SubversionSccName + "Pending";
        const string SccEnlistStream = VisualGitId.SubversionSccName + "Enlist";
                ...
                using (ComStreamWrapper wrapper = new ComStreamWrapper(pOptionsStream))
                {
                    ...
                    switch (pszKey)
                    {
                        case SccPendingChangeStream:
                            ...
                            break;
                        case SccEnlistStream:
                            scc = GetService<IVisualGitSccService>();
                            if (scc != null)
